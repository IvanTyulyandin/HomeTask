...
abstract class TweetSet {
  ...
  def trending(words : List[String]): TweetList ={
    this.filter(tweet => words.exists(word => tweet.text.contains(word))).descendingByRetweet
  }
}
...
class Empty extends TweetSet {
  ...
  def mostRetweeted = throw new java.util.NoSuchElementException("No twits")
  ...
  def mostRetweeted = {
    ...
    if (right.isInstanceOf[Empty]) {
      if (left.isInstanceOf[Empty])
        elem
      else max(left.mostRetweeted, elem)
    }
    else {
      if (left.isInstanceOf[Empty]){
        max(right.mostRetweeted, elem)
      }
      else max(right.mostRetweeted, left.mostRetweeted)
    }
  }

  def descendingByRetweet ={
    val mostRetw = this.mostRetweeted
